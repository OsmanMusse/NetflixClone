...
extension Firebase.Database {
    ...
    static func addToMyList(videoInfo: VideoData){
        ...
        Firebase.Database.getActiveUser { (profileData) in
            ...
            guard let videoTitle = videoInfo.videoTitle else {return}
            guard let videoURL = videoInfo.videoURL else {return}
            guard let randomKey = videoInfo.videoKeyIdentifier else {return}
    ...
    static func isVideoInMyList(videoInfo: VideoData, completion: @escaping (Bool) -> Void){
        guard let userID = Firebase.Auth.auth().currentUser?.uid else {return}
        guard let videoKey = videoInfo.videoKeyIdentifier else {return}
        
        getActiveUser { (profileData) in
            guard let profileIdentifier = profileData.userIdentifier else {return}
            Firebase.Database.database().reference().child("MyList").child(userID).child(profileIdentifier).child(videoKey).observeSingleEvent(of: .value) { (snapShot) in
                ...
                if categoryName == "Trending Now" {
                    for videos in categoryVideos {
                        ...
                        for videoTrailer in videoTrailerDict {
                            ...
                            guard let videoTrailerKey = videoTrailerArray["videoIdentifier"] as? String else {return}
                            videoInfo.videoTitle = videoTrailerTitle
                            videoInfo.videoURL = videoTrailerURL
                            videoInfo.videoKeyIdentifier = videoTrailerKey
    ...
    static func getPreviewVideos(completion: @escaping ([VideoData]) -> Void){
        Firebase.Database.database().reference().child("Previews").child("videoData").observeSingleEvent(of: .value) { (snapShot) in
            guard let videoDict = snapShot.value as? [String: Any] else {return}
            var videoArray: [VideoData] = []
