...
class VideoViewCell: UICollectionViewCell {
    ...
    let posterImage = CustomImageView(image: #imageLiteral(resourceName: "The-red-sea-poster"))
    ...
    func fetchVideoInfo(){
        Firebase.Database.getRandomTrailerVideo { (videoData) in
            self.videoTrailerData = videoData
            Firebase.Database.isVideoInMyList(videoInfo: self.videoTrailerData) { (videoValue) in
                if videoValue == true {
                    DispatchQueue.main.async {
                        self.listButton.setImage(#imageLiteral(resourceName: "tick").withRenderingMode(.alwaysOriginal), for: .normal)
                        self.isToggled = true
                    }
    ...
    func fetchObservers() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(switchVideoOff), name: NotificationName.didHomeScreenScrollUp.name, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(switchVideoOn), name: NotificationName.didHomeScreenScrollDown.name, object: nil)
    }
    ...
    @objc func switchVideoOff(){
        let containerViewFrame = containerView.frame.origin
        player?.pause()
        containerView.addSubview(posterImage)
        posterImage.frame = CGRect(x: containerViewFrame.x, y: containerViewFrame.y, width: containerView.frame.width, height: containerView.frame.height - 50)
        
    }
    
    @objc func switchVideoOn(){
